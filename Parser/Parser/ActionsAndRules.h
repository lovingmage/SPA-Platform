...
public:
  ...
  void doAction(ITokCollection*& pTc)
  {
    ...
    if(elem.type == "sfunction")
    {
      //std::cout << "\nHandlePop";
      //std::cout << "\n--popping at line count = " << p_Repos->lineCount();
      std::cout << "\n  Static Function " << elem.name << ", lines = " << p_Repos->lineCount() - elem.lineCount + 1;
      std::cout << "\n";
    }
	else if (elem.type == "nfunction")
	{
		std::cout << "\n  Non-Static Function " << elem.name << ", lines = " << p_Repos->lineCount() - elem.lineCount + 1;
		std::cout << "\n";
	}
	else if (elem.type == "scope")
	{
		std::cout << "\n  Scope " << elem.name << ", lines = " << p_Repos->lineCount() - elem.lineCount + 1;
		std::cout << "\n";
	}
	...
	bool doTest(ITokCollection*& pTc)
	{
		...
		if (tc[tc.length() - 1] == "{")
		{
			const static std::string keys[]
				= { "for", "while", "switch", "if", "catch", "static", "namespace" };
			for (int i = 0; i < 7; ++i)
			{
...
public:
  ...
  bool doTest(ITokCollection*& pTc)
  {
    ...
    {
      size_t len = tc.find("(");
      if(len < tc.length() && !isSpecialKeyWord(tc[len-1]) && tc[0] != "static")
      {
        doActions(pTc);
        return true;
      }
...
public:
  ...
  void doAction(ITokCollection*& pTc)
  {
    std::cout << "\n  Non-Static FuncDef: " << pTc->show().c_str();
  }
};
...
///////////////////////////////////////////////////////////////
// rule to detect static function definitions

class StatFunctionDefinition : public IRule
{
public:
	bool isSpecialKeyWord(const std::string& tok)
	{
		const static std::string keys[]
			= { "for", "while", "switch", "if", "catch", "namespace" };
		for (int i = 0; i<6; ++i)
			if (tok == keys[i])
				return true;
		return false;
	...
	bool doTest(ITokCollection*& pTc)
	{
		...
		{
			size_t len = tc.find("(");
			size_t lenstat = tc.find("static");
			if (lenstat < tc.length() && len < tc.length() && !isSpecialKeyWord(tc[len - 1]))
			{
				doActions(pTc);
				return true;
			}
...
///////////////////////////////////////////////////////////////
// action to push static function name onto ScopeStack

class StatPushFunction : public IAction
{
	Repository* p_Repos;
public:
	StatPushFunction(Repository* pRepos)
	{
		p_Repos = pRepos;
	}
	void doAction(ITokCollection*& pTc)
...
///////////////////////////////////////////////////////////////
// action to send semi-expression that starts a static function def
// to console

class StatPrintFunction : public IAction
{
	Repository* p_Repos;
public:
	StatPrintFunction(Repository* pRepos)
	{
		p_Repos = pRepos;
	}
	void doAction(ITokCollection*& pTc)
	{
		std::cout << "\n  Static FuncDef: " << pTc->show().c_str();
	}
};
